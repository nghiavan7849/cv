							...
							<nav class="pagination-outer" aria-label="Page navigation">
							    <ul class="pagination">
								    <li class="page-item ${currentPage <= 1 ? 'disabled' : ''}">
								        <a href="/adminIndex/danh-gia?pageNo=${currentPage - 1}&pageSize=${pageSize}&search=${search}&sort=${sort}&ngayFrom=<fmt:formatDate value='${ngayFrom}' pattern='yyyy-MM-dd'/>&ngayTo=<fmt:formatDate value='${ngayTo}' pattern='yyyy-MM-dd'/>" class="page-link" aria-label="Previous">
								            <span aria-hidden="true">«</span>
								        </a>
								    </li>
								    <c:forEach var="page" begin="1" end="${totalPages}">
								        <li class="page-item ${currentPage == page ? 'active' : ''}">
								            <a class="page-link" href="/adminIndex/danh-gia?pageNo=${page}&pageSize=${pageSize}&search=${search}&sort=${sort}&ngayFrom=<fmt:formatDate value='${ngayFrom}' pattern='yyyy-MM-dd'/>&ngayTo=<fmt:formatDate value='${ngayTo}' pattern='yyyy-MM-dd'/>">${page}</a>
								        </li>
								    </c:forEach>
								    <li class="page-item ${(totalPages == 0 || currentPage >= totalPages) ? 'disabled' : ''}">
								        <a href="/adminIndex/danh-gia?pageNo=${currentPage + 1}&pageSize=${pageSize}&search=${search}&sort=${sort}&ngayFrom=<fmt:formatDate value='${ngayFrom}' pattern='yyyy-MM-dd'/>&ngayTo=<fmt:formatDate value='${ngayTo}' pattern='yyyy-MM-dd'/>" class="page-link" aria-label="Next">
								            <span aria-hidden="true">»</span>
								        </a>
								    </li>
								</ul>
